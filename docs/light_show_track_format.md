...
A curated set of twelve demonstration files is available under
`tracks/*.lstf`. They are generated by `scripts/generate_demo_lstf_tracks.py`
and cover effects such as palette-driven fades, synchronous strobes, countdown
timers, tempo ramps, and palette overrides. Each file contains centre, left,
and right pad timelines lasting between five and fifteen seconds so they can be
looped directly by the LSTF player.
...
### Text transport wrapper

To keep demo tracks version-control friendly, repositories store LSTF payloads
inside a simple ASCII envelope.  The textual form begins with the header line
`LSTF-TEXT 1` followed by base64-encoded chunk data (wrapped at 76 columns for
readability).  Players first strip the header, decode the base64 body, then
parse the resulting binary stream as described below.  For compatibility, tools
may accept either the wrapped text representation or a raw binary stream.
